 ...
 * Restores dirty candidate tree to preCycleSha when present, then commits
 * control-plane bookkeeping so the tree stays beginCycle-clean when re-enabled.
 */

import { patchState, readState } from './lib/state.mjs';
import { rejectAndRestore, headSha, commitPaths, CONTROL_PLANE_PATHS, verifyRepoGuards } from './lib/cycle.mjs';
...
async function main() {
  ...
  const committed = commitPaths(
    [...CONTROL_PLANE_PATHS],
    'chore(autoqa): deactivate autonomous improvement'
  );
  if (!committed.ok) {
    console.error(JSON.stringify({
      ok: false,
      error: 'failed to commit deactivation control plane',
