...
function rawGuidanceFor(f,environment={type:'unknown'}){
  ...
  if(/lang-missing/.test(id))return{interpretation:'The root html element has no lang attribute declaring the page language.',impact:'Assistive technology may use the wrong pronunciation rules when the document language is not declared.',remediation:'Set a valid BCP 47 language code on the root html element, such as lang="en" or the appropriate language for the page.',verify:'Rescan and confirm the root html element exposes the intended valid language.'};
  ...
  if(id==='web.image-broken')return{interpretation:'An image element finished loading with naturalWidth 0, which usually means the resource is missing or undecodable.',impact:'Broken images create visible gaps and can remove meaningful content.',recommendation:'Restore the image resource or remove the unused img element.',remediation:'Confirm the src resolves, fix the path or CDN object, or remove the image if it is obsolete.',verify:'Reload and confirm the image paints with non-zero natural dimensions, then rescan.'};
