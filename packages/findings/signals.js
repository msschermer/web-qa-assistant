...
export function signalForFinding(f={}){
  ...
  if(/broken[- ]link|link[- ].*(404|410|5\d\d)|\b404\b|\b410\b/.test(text))return SIGNALS.BROKEN_LINK;
  if(/link.*timeout|could not be verified/.test(text))return SIGNALS.LINK_REVIEW;
