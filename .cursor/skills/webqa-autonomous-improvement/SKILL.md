...
Do **not** require Playwright's bundled Chromium. Missing bundled Chromium is not a blocker when Google Chrome is installed. On modern branded Chrome (137+), AutoQA spawns installed Chrome with `--enable-unsafe-extension-debugging`, connects over CDP, and loads `dist/extension` via `Extensions.loadUnpacked` into the dedicated persistent profile `.autoqa/chrome-profile/` (gitignored). Optional `http://*/*` / `https://*/*` access is granted once via `node tools/autoqa/chrome-profile-bootstrap.mjs` (human Grant/Allow only — no terminal Enter). Chrome 151+ deletes CDP `loadUnpacked` installs on the next startup (`INSTALLED_VIA_CDP`); bootstrap clears that flag and finalizes a durable unpacked install via `chrome.developerPrivate.reload`, then verifies permissions across a second Chrome session with graceful CDP `Browser.close` + child-exit wait. Later dogfood reuses the durable profile install and never calls `permissions.request`. Revalidate browser capability only when Chrome is missing or launch/extension load fails.
