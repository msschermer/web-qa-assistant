...
async function scan(html, { url = 'https://example.com/frame-interaction', resources = [], wireToggles = true, prepare = true } = {}) {
  ...
  if (wireToggles) {
    ...
    for (const frame of dom.window.document.querySelectorAll('iframe')) {
      try {
        const doc = frame.contentDocument;
        const ft = doc?.getElementById('frame-toggle');
        if (!ft) continue;
        ft.addEventListener('click', function () {
          const open = this.getAttribute('aria-expanded') === 'true';
          this.setAttribute('aria-expanded', open ? 'false' : 'true');
          const panel = doc.getElementById(this.getAttribute('aria-controls'));
          if (panel) panel.hidden = open;
        });
      } catch {}
    }
...
test('quiet and interaction-clean controls stay free of toggle failures', async () => {
  const quiet = await scan(quietHtml);
  assert.equal(ruleIds(quiet.report).includes('ux.disclosure-toggle-failed'), false);
  const clean = await scan(cleanHtml);
...
test('broken disclosure fails safe interaction check; working one passes and restores', async () => {
  const { report, dom } = await scan(frameHtml);
  ...
  assert.equal(failed.interactionObservation?.restoredState?.restored ?? failed.interactionObservation?.restored, true);
...
test('same-origin iframe interior form and lang issues are scoped with frame metadata', async () => {
  const { report, dom } = await scan(frameHtml);
  ...
  // Title was added to fixture; lang should still be missing unless set.
  ...
  const frameToggle = dom.window.document.getElementById('so-frame')?.contentDocument?.getElementById('frame-toggle');
  if (frameToggle) assert.equal(frameToggle.getAttribute('aria-expanded'), 'false');
});

test('cross-origin iframe is coverage-only without interior findings', async () => {
  const { report } = await scan(frameHtml);
...
test('tracking cross-origin failures stay diagnostic; visible third-party script is Worth Checking', async () => {
  const resources = [
  ...
  const { report } = await scan(html, { url: 'https://example.com/resources', resources, wireToggles: false });
  const ids = ruleIds(report);
...
test('script failure plus toggle failure correlate without claiming causation', async () => {
  const resources = [{
  ...
  const { report } = await scan(frameHtml, { resources });
  ...
  assert.match(guidanceFor(toggle).limitations, /not as proven causation|Worth Checking|allowlisted reversible/i);
});

test('PSI remains deferred in scan metadata', async () => {
  const { report } = await scan(cleanHtml);
