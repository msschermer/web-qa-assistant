...
import { RuntimeTrace, buildBugReport, bugReportPrivacySummary } from '../packages/support/bug-report.js';
...
test('Report bug is privacy-safe by default and does not include page/Frank content',()=>{
  ...
  assert.equal(artifact.schema,'web-qa-assistant-bug-report/v1');
  ...
  assert.match(bugReportPrivacySummary(false),/Page content, selectors, Frank wording/);
});
