...
test('browser performance guidance is metric-specific and evidence-led',()=>{
  ...
  const lcp=guidanceFor({ruleId:'performance.browser.lcp',performanceObservation:{largestContentfulPaintMs:5100,lcpElement:{selector:'#hero'}}});assert.doesNotMatch(lcp.interpretation,/#hero/);assert.match(lcp.interpretation,/evidence record/i);assert.match(lcp.remediation,/LCP element|image|text/i);
