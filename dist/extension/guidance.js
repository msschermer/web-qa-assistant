...
import { linkInTextEvidence, linkInTextDiagnosis, linkInTextRemediation } from './link-in-text.js';
...
function imageAdvice(f){
  ...
  return{
    ...
    alternatives:'Both branches are shown because the page evidence genuinely supports either one.',
...
function constrainVerifyToCheckedSources(verify,pub){
  ...
  text=text.replace(/then rescan with Meta State(?: and Frank)?\.?/gi,'then rescan.');
...
function rawGuidanceFor(f,environment={type:'unknown'}){
  ...
  if(/link-review/.test(id)){
    ...
    return{
      interpretation:`${label} was checked, but the result is inconclusive.${statusNote} Lumen is not treating this as a confirmed broken link.`,
      impact:`Visitors may still reach the destination, or they may hit ${kind}. Without a confirmed missing-page or server-error response, this scan cannot claim the link is broken.`,
  ...
  if(id==='performance.browser.lcp-heavy-image'||id==='performance.browser.lcp-image-oversized'){
    ...
    if(m.intrinsicWidth&&m.renderedWidth&&m.requiredPhysicalWidth){
      const transfer=Number.isFinite(m.transferBytes)?` It transferred about ${(m.transferBytes/1024).toFixed(0)} KB in this lab observation.`:'';
      const responsive=m.responsiveSourcePresent
        ?' Responsive markup (srcset/picture) is present, but the selected candidate is still larger than the DPR-adjusted need. Check sizes and candidate widths.'
        :' No srcset/picture set was observed; serve an appropriately sized source or add responsive candidates.';
      const wp=f.remediationContext?.platform==='wordpress'
        ?' Fix the source and responsive delivery first. On WordPress, an image optimization plugin can automate parts of that workflow after the underlying delivery is correct.'
        :'';
      return{
        interpretation:`This is the current LCP image. The selected source is ${m.intrinsicWidth}×${m.intrinsicHeight}, while it renders around ${m.renderedWidth}×${m.renderedHeight}. At this display’s ${m.devicePixelRatio}× pixel density, roughly ${m.requiredPhysicalWidth}×${m.requiredPhysicalHeight} would be sufficient, so the selected source still contains substantially more image data than needed.${transfer}${p.largestContentfulPaintMs!=null?` Lab LCP was ${(p.largestContentfulPaintMs/1000).toFixed(1)}s.`:''}`,
        impact:'Because this image is the LCP element, oversized delivery has higher performance value than secondary oversized images on the page. This remains a lab observation, not a field Core Web Vitals claim.',
        recommendation:'Serve a more appropriate responsive source for this LCP image and verify srcset/sizes select a DPR-fit candidate.',
        remediation:`Resize or re-encode the LCP asset for the DPR-adjusted display size, then verify the browser selects that candidate via srcset/sizes.${responsive}${wp}`,
        verify:'Rescan and confirm the LCP image finding clears or the selected candidate dimensions approach the DPR-adjusted need without layout regression.',
        limitations:'Dimension mismatch is measured. Transfer and LCP timing, when present, support impact inference but do not prove a historical field regression.'
      };
    }
  ...
  if(/ux\.disclosure-toggle-failed|ux\.menu-toggle-failed/.test(id)){
    ...
    return{
      ...
      limitations:inFrame
        ?'Coverage limitation: allowlisted activation is not zero side effects, and page handlers may still run. This control lives inside a same-origin iframe; spotlight may be unavailable. Forms, navigation, purchases, downloads, and third-party widgets are never activated. Co-occurring script failures are correlated only as Worth Checking, not as proven causation.'
        :'Coverage limitation: allowlisted activation is not zero side effects, and page handlers may still run (analytics, fetches, preference writes). Forms, navigation, purchases, downloads, and third-party widgets are never activated. Co-occurring script failures are correlated only as Worth Checking, not as proven causation.'
    };
  }
  ...
  if(id==='performance.browser.cls')return{interpretation:`This browser observed cumulative layout shift of ${f.performanceObservation?.cumulativeLayoutShift??'an elevated value'} on this machine and network. This is a lab observation, not a field score or Core Web Vitals result. WebQA recorded the aggregate CLS value only; it did not observe which element contributed the shift.`,impact:'Unstable layout can make the page feel jumpy and can cause accidental clicks. Without a measured shift contributor, do not treat any specific banner, image, or font as the confirmed cause.',recommendation:'Use DevTools Layout Shift regions in this lab session to identify contributors, then fix the observed source.',remediation:'WebQA cannot name the shifting element from aggregate CLS alone. In Chrome Performance/Experience tools, inspect Layout Shift attributions for this navigation. Common causes to check once attributed: images without dimensions, late-injected UI, and font swap. Do not treat this as field Core Web Vitals or a historical regression without monitored field/history evidence.',verify:'Repeat the navigation under comparable conditions and confirm the lab CLS value decreases; compare with monitored field/history data only when that evidence is present.',limitations:'Aggregate CLS only; no layout-shift contributor nodes were captured in this scan.'};
  ...
  return{impact:'This finding is supported by the current scan, but it does not yet have enough rule-specific context to claim a precise implementation impact.',remediation:'Review the evidence and source tool before changing code. Do not prescribe a generic implementation change when the available evidence does not identify one safely.',verify:'Rescan after any intentional change and confirm the original evidence no longer reproduces.'};
}
