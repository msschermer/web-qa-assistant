...
export function collectDomSignals(source = {}) {
...
export function assessTargetIntegrity(input = {}) {
...
export function targetIntegrityReached(integrity) {
  return !integrity || integrity.state === TARGET_STATES.REACHED;
}

export function targetIntegrityLimitsAudit(integrity) {
  return Boolean(integrity && integrity.state !== TARGET_STATES.REACHED);
}

export function targetIntegrityBlocksAudit(integrity) {
...
export function isPageDerivedFinding(finding = {}) {
  const ruleId = String(finding.ruleId || '');
  return PAGE_DERIVED_RULE_PREFIXES.some(prefix => ruleId.startsWith(prefix));
}

export function suppressFindingsForTargetIntegrity(findings = [], integrity) {
...
export function adjustCoverageForTargetIntegrity(coverage = {}, linkAudit = null, integrity) {
  const next = { ...coverage };
...
export function targetIntegrityBrief(integrity) {
